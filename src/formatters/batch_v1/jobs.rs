...
const CRONJOB_CONTAINERS: &str = "spec.jobTemplate.spec.template.spec.containers";
...
impl ResourceFormatter for JobFormatter {
    ...
    fn format_wide(&self, item: &DynamicObject, span: Span) -> Value {
        ...
        rec.push(
            "containers",
            fmt_containers(json_array(&item.data, CRONJOB_CONTAINERS), span),
        );
        rec.push(
            "images",
            fmt_images(json_array(&item.data, CRONJOB_CONTAINERS), span),
        );
        rec.push(
            "restartPolicy",
            Value::string(json_str(data, "spec.template.spec.restartPolicy"), span),
        );
