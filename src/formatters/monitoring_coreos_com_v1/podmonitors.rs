//! Formatter for `monitoring.coreos.com/v1 PodMonitor` resources.
...
pub struct PodMonitorFormatter;
...
fn pod_metrics_endpoints_spec(item: &DynamicObject, span: Span) -> Value {
    let rows: Vec<Value> = json_array(&item.data, &["spec", "podMetricsEndpoints"])
        .iter()
        .map(|e| {
...
impl ResourceFormatter for PodMonitorFormatter {
    fn format_compact(&self, item: &DynamicObject, span: Span) -> Value {
        let endpoints_count =
            json_array(&item.data, &["spec", "podMetricsEndpoints"]).len() as i64;
    ...
    fn format_wide(&self, item: &DynamicObject, span: Span) -> Value {
        ...
        rec.push("endpointsSpec", pod_metrics_endpoints_spec(item, span));
